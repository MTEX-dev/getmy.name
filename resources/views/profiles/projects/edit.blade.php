    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="post" action="{{ route('profiles.projects.update', $project) }}" class="mt-6 space-y-6" enctype="multipart/form-data">
                    @csrf
                    @method('patch')
                    ...
                    <div>
                        ...
                        <x-textarea id="description" name="description" class="mt-1 block w-full">{{ old('description', $project->description) }}</x-textarea>
                        <x-input-error class="mt-2" :messages="$errors->get('description')" />
                    </div>

                    <div>
                        <x-input-label for="url" :value="__('Project URL (Optional)')" />
                        <x-text-input id="url" name="url" type="url" class="mt-1 block w-full" :value="old('url', $project->url)" placeholder="https://example.com/project" />
                    ...
                    <div>
                        ...
                        @if ($project->image_path)
                            ...
                            <img src="{{ $project->imageUrl() }}" alt="{{ $project->name }}" class="mb-4 w-48 h-32 object-cover rounded-md" />
                        @endif
                        <input
                            id="image"
                            name="image"
